...
class CopyDocument404ResponseTest extends AnyWordSpec with Matchers {

  "CopyDocument404Response.fromJson" should {
    """not parse invalid json""" in {
      val Failure(err) = Try(CopyDocument404ResponseData.fromJsonString("invalid jason"))
      err.getMessage should startWith("Error parsing json 'invalid jason'")
    }
    """parse """ ignore {
      val Failure(err: ValidationErrors) =
        CopyDocument404ResponseData.fromJsonString("""""").validated()
