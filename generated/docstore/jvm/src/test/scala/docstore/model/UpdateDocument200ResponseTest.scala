...
class UpdateDocument200ResponseTest extends AnyWordSpec with Matchers {

  "UpdateDocument200Response.fromJson" should {
    """not parse invalid json""" in {
      val Failure(err) = Try(UpdateDocument200ResponseData.fromJsonString("invalid jason"))
      err.getMessage should startWith("Error parsing json 'invalid jason'")
    }
    """parse """ ignore {
      val Failure(err: ValidationErrors) =
        UpdateDocument200ResponseData.fromJsonString("""""").validated()
