...
class SaveDocument200ResponseTest extends AnyWordSpec with Matchers {

  "SaveDocument200Response.fromJson" should {
    """not parse invalid json""" in {
      val Failure(err) = Try(SaveDocument200ResponseData.fromJsonString("invalid jason"))
      err.getMessage should startWith("Error parsing json 'invalid jason'")
    }
    """parse """ ignore {
      val Failure(err: ValidationErrors) =
        SaveDocument200ResponseData.fromJsonString("""""").validated()
