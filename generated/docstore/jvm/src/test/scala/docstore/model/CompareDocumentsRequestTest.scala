...
class CompareDocumentsRequestTest extends AnyWordSpec with Matchers {

  "CompareDocumentsRequest.fromJson" should {
    """not parse invalid json""" in {
      val Failure(err) = Try(CompareDocumentsRequestData.fromJsonString("invalid jason"))
      err.getMessage should startWith("Error parsing json 'invalid jason'")
    }
    """parse """ ignore {
      val Failure(err: ValidationErrors) =
        CompareDocumentsRequestData.fromJsonString("""""").validated()
